...
app.post("/tts", async (req, res) => {
  try {
    ...
    if (!FISH_VOICE_ID) {
      console.error("FISH_VOICE_ID is not set in environment variables!");
      return res.status(500).send("Server misconfigured: missing voice ID");
    }
    if (!FISH_AUDIO_API_KEY) {
      console.error("FISH_AUDIO_API_KEY is not set in environment variables!");
    ...
    // and REQUIRES a "model" header on every request.
    const requestBody = msgpack.encode({
      text: textToSpeak,
      reference_id: String(FISH_VOICE_ID),
    ...
    const fishResponse = await fetch("https://api.fish.audio/v1/tts", {
      ...
      headers: {
        ...
        "model": "s2-pro", // REQUIRED header — Fish Audio rejects requests without this
      },
      body: requestBody,
    });
